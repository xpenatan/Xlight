...
import xlight.editor.imgui.ecs.system.XImGuiSystem;
...
public class XImGuiSystemImpl implements XSystem, XImGuiSystem {
    ...
    ImGuiWindowClass rootWindowClass;

    XECSWorld editorEngine;

    public XImGuiSystemImpl() {
        windowContexts = new XIntMap<>();
    }

    @Override
    public void onAttach(XECSWorld world) {
        this.editorEngine = world;
        XInitFeatureService service = world.getService(XInitFeatureService.class);
        service.addFeatureDependency(() -> initImGui(world), XImGuiManager.FEATURE);
    }
        ...
        if(init) {
            ...
            if(imGuiManager != null) {
                ...
                for(XIntMapListNode<XImGuiWindowContext> cur : list) {
                    ...
                    int key = cur.getKey();
                    window.render(editorEngine, rootDockspaceID, key);
                }
