...
public class XBaseSpriteBatch implements XBatch2D {

    public static float Z_VALUE = 0;
     ...
     * @see SpriteBatch#SpriteBatch(int, ShaderProgram)
     */
    public XBaseSpriteBatch(int size) {
        this(size, null);
    }
    ...
    public static final int ORIGINAL_SPRITE_SIZE = 4 * (2 + 1 + 2);
     ...
     */
    public XBaseSpriteBatch(int size, ShaderProgram defaultShader) {
    ...
    public void drawSprite(Texture texture, float[] spriteVertices, int offset, int count, float regionWidth, float regionHeight) {
        ...
        drawSprite(spriteVertices, offset, idx, copyCount);
...
//            System.arraycopy(spriteVertices, offset, vertices, 0, copyCount);
            drawSprite(spriteVertices, offset, 0, copyCount);
    ...
    private void drawSprite(float[] spriteVertices, int offset, int outOffset, int size) {
        int curIdx = outOffset;
        for(int i = 0; i < size; i = i + 5) {
            int index = curIdx + i;
            int spriteIndex = offset + i;
            curIdx++;
            idx++;
            vertices[index] = spriteVertices[spriteIndex];     // X
            vertices[index + 1] = spriteVertices[spriteIndex + 1]; // Y
            vertices[index + 2] = enableZ ? Z_VALUE : 0f;         // Z
            vertices[index + 3] = spriteVertices[spriteIndex + 2]; // Color
            vertices[index + 4] = spriteVertices[spriteIndex + 3]; // U
            vertices[index + 5] = spriteVertices[spriteIndex + 4]; // V
        }
    }
    ...
    public void flush() {
        ...
        renderCalls++;
        totalRenderCalls++;
        // changed 20 to 24 to contains Z values
